...
public class PagingUtil {
	...
	/**
	 * currentPage : 현재페이지 totalCount : 전체 게시물 수 rowCount : 한 페이지의 게시물의 수 pageCount
	 * : 한 화면에 보여줄 페이지 수 pageUrl : 호출 페이지 url addKey : 부가적인 key 없을 때는 null 처리
	 * (&num=23형식으로 전달할 것)
	 */
	...
	public PagingUtil(String keyfield, String keyword, int currentPage, int totalCount, int rowCount, int pageCount,
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				pagingHtml.append("&nbsp;<b> <font color='#FE9A2E'>");
				pagingHtml.append(i);
